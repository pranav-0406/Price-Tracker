...
const retailerColors = { Amazon: '#e58442', Flipkart: '#437ac8', Croma: '#4aa46f', 'Reliance Digital': '#147dff', 'Vijay Sales': '#e84646', Sennheiser: '#9b7653' }
...
function App() {
  ...
  useEffect(() => { loadData(); const timer = window.setInterval(loadData, 1800000); return () => window.clearInterval(timer) }, [loadData])
  useEffect(() => {
    if (!selectedProductId) return
  ...
  const filteredRows = useMemo(() => priceRows.filter((item) => colorFilter === 'selected' ? item.colorValue === selectedColor : colorFilter === 'all' || item.colorValue === colorFilter).filter((item) => platformFilter === 'all' || item.platform === platformFilter).filter((item) => !minPrice || (Number.isFinite(item.price) && item.price >= Number(minPrice))).filter((item) => !maxPrice || (Number.isFinite(item.price) && item.price <= Number(maxPrice))).sort((a, b) => {
    if (!Number.isFinite(a.price)) return 1
    if (!Number.isFinite(b.price)) return -1
    return sort === 'price-desc' ? b.price - a.price : sort === 'platform' ? a.platform.localeCompare(b.platform) : sort === 'color' ? a.color.localeCompare(b.color) : a.price - b.price
  }), [priceRows, selectedColor, platformFilter, colorFilter, minPrice, maxPrice, sort])
  const bestDealRows = useMemo(() => filteredRows.filter((item) => item.platform !== 'Sennheiser' && item.verified && Number.isFinite(item.price)), [filteredRows])
  const liveRetailers = useMemo(() => [...new Set(priceRows.filter((item) => item.verified && item.dataMode === 'live').map((item) => item.platform))], [priceRows])
  const liveStatus = liveRetailers.length ? `Live: ${liveRetailers.join(', ')}` : 'Live sources pending'
  ...
  const priceIsVerified = filteredRows.length > 0 && filteredRows.every((item) => item.verified)
  const rowPriceLabel = (item) => item.verified && Number.isFinite(item.price) ? money(item.price) : item.statusReason === 'retailer not yet supported in v1' ? 'Coming soon' : item.statusReason === 'pending Flipkart API compatibility verification' ? 'Pending verification' : 'Not verified'
  ...
  const addNewProduct = async (event) => {
    ...
    setProducts((current) => current.map((item) => item.id === data.product.id ? data.product : item)); setSelectedProductId(data.product.id); setShowAddProduct(false); setNewProduct({ url: '', targetPrice: '' })
  }
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><button className="mobile-menu" onClick={() => setMobileNav(true)}><Menu size={21} /></button><div className="breadcrumbs"><span>Products</span><ChevronRight size={14} /><b>{product?.name || 'Price tracker'}</b></div><div className="topbar-actions"><div className="search"><Search size={16} /><span>Search products</span></div><button className="icon-button" onClick={() => setShowAlert(true)}><BellRing size={18} /></button><button className="avatar top-avatar">PS</button></div></header>
      <div id="overview" className="content">{error && <div className="error-banner">{error}<button onClick={() => setError('')}><X size={14} /></button></div>}<section className="product-heading"><div className="heading-main"><div className="product-art">{imageFailed || !product?.image ? <Headphones size={56} /> : <img src={product.image} alt={`${product.name} product`} onError={() => setImageFailed(true)} />}</div><div><div className="eyebrow"><span className="live-dot" /> {liveStatus}</div><h1>{product?.name || 'Loading tracker…'}</h1><p>{product?.description}</p><div className="tag-row"><span className="tag">SKU: {product?.sku || '—'}</span><span className="updated"><Clock3 size={13} /> {loading ? 'Refreshing…' : lastSync ? `Updated ${new Date(lastSync).toLocaleTimeString()}` : 'Waiting for sync'}</span></div></div></div><button className="alert-button" onClick={() => setShowAlert(true)}><Bell size={17} /> Set price alert</button></section>
        ...
        <div className="dashboard-grid">                <section id="history" className="card chart-card"><div className="section-header"><div><h2>Price history</h2><p>{activeColor?.name || 'Selected color'} · {range === '1W' ? 'Last 7 days' : range === '1M' ? 'Last 30 days' : 'Last 90 days'}</p></div><div className="range-tabs">{['1W', '1M', '3M'].map((item) => <button key={item} className={range === item ? 'selected' : ''} onClick={() => setRange(item)}>{item}</button>)}</div></div>{dataMode !== 'live' ? <div className="no-history">Verified history will appear after live retailer sources are connected.</div> : <><div className="chart-legend">{Object.keys(retailerColors).map((name) => <span key={name}><i className="legend-line" style={{ background: retailerColors[name] }} /> {name}</span>)}</div><div className="chart-wrap"><ResponsiveContainer width="100%" height={245}><AreaChart data={history}><CartesianGrid stroke="#e8e8e2" vertical={false} /><XAxis dataKey="date" hide={history.length > 30} /><YAxis domain={['auto', 'auto']} tickFormatter={(value) => `₹${Math.round(value / 1000)}k`} /><Tooltip formatter={(value) => money(value)} /><Area type="monotone" dataKey="Amazon" stroke={retailerColors.Amazon} fill="#e5844220" strokeWidth={2.5} /><Area type="monotone" dataKey="Flipkart" stroke={retailerColors.Flipkart} fill="none" /><Area type="monotone" dataKey="Croma" stroke={retailerColors.Croma} fill="none" /><Area type="monotone" dataKey="Reliance Digital" stroke={retailerColors['Reliance Digital']} fill="none" /><Area type="monotone" dataKey="Vijay Sales" stroke={retailerColors['Vijay Sales']} fill="none" /><Area type="monotone" dataKey="Sennheiser" stroke={retailerColors.Sennheiser} fill="none" /></AreaChart></ResponsiveContainer></div></>}<div className="chart-footer"><span><TrendingDown size={15} /> <b>Historical low:</b> {historicalLow ? money(historicalLow) : 'Not available'}</span></div></section>
    ...
    {showAddProduct && <div className="modal-backdrop" onClick={() => setShowAddProduct(false)}><div className="modal small" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setShowAddProduct(false)}><X size={18} /></button><h2>Add a product URL</h2><form onSubmit={addNewProduct} className="add-product-form"><label>Retailer product URL<input required value={newProduct.url} type="url" placeholder="https://www.flipkart.com/..." onChange={(event) => setNewProduct({ ...newProduct, url: event.target.value })} /></label><label>Optional target price<input value={newProduct.targetPrice} type="number" min="1" placeholder="₹" onChange={(event) => setNewProduct({ ...newProduct, targetPrice: event.target.value })} /></label><button type="submit" className="save-alert">Track URL</button></form></div></div>}
