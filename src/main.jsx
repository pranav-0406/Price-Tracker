...
function App() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}><div className="brand"><span className="brand-mark"><Headphones size={18} /></span><span>track<span className="brand-dot">.</span>audio</span></div><div className="workspace-label">YOUR WORKSPACE</div><nav><a className="nav-item active"><LayoutDashboard size={18} /> Overview</a><a className="nav-item"><LineChartIcon size={18} /> Price history</a><a className="nav-item"><Bell size={18} /> My alerts <span className="nav-count">{alerts.length}</span></a><a className="nav-item"><ShoppingBag size={18} /> Retailers</a></nav><div className="sidebar-divider" /><div className="workspace-label">TRACKED PRODUCTS</div><div className="product-list">{products.map((item) => <button key={item.id} className={`product-link ${selectedProductId === item.id ? 'selected' : ''}`} onClick={() => { setSelectedProductId(item.id); setMobileNav(false) }}><span className="mini-product"><Headphones size={14} /></span><span><b>{item.name}</b><small>{item.brand}</small></span></button>)}</div><button className="add-product" onClick={() => setShowAddProduct(true)}><span>+</span> Add product</button><div className="sidebar-bottom"><div className="sync-card"><div className="sync-title"><span className="live-dot" /> Auto-refresh every 5 min</div><small>{dataMode === 'demo' ? 'Demo data · scraper not connected' : 'Live retailer data'}</small></div><a className="nav-item"><Settings size={18} /> Settings</a><a className="nav-item"><CircleHelp size={18} /> Help center</a><div className="profile"><div className="avatar">PS</div><div><b>Pranav S.</b><small>Free plan</small></div><ChevronDown size={14} /></div></div></aside>
